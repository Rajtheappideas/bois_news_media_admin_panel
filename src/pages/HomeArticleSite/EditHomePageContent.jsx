...
import {
  handleGetHomePageConentById,
  handleUpdateHomePageConent,
} from "../../redux/HomeArticleSiteSlice";
...
const EditHomePageContent = () => {
  ...
  const { homePageContentUpdateLoading, homePageLoading, singleContent } =
    useSelector((state) => state.root.homepagecontent);
  ...
  const {
    ...
    defaultValues: { website: singleContent?.website },
  });
  ...
  const handleAddImageAndArticles = (data, from) => {
    ...
    if (findData) {
      if (from === "herosection") {
        if (heroSection.some((content) => content.order == data?.name)) {
          setHeroSection((prev) =>
            prev.map((content) =>
              content?.order == data?.name ? findData : content
            )
          );
        } else {
          setHeroSection([...heroSection, { ...findData, order: data.name }]);
        }
      } else {
        if (otherSections.some((content) => content.order == data?.name)) {
  ...
  useEffect(() => {
    dispatch(handleGetHomePageConentById({ token, id })).then((res) => {
      if (res.payload.status === "success") {
        const herosectionData = res.payload?.content?.heroSection.map(
          (c, i) => {
            if (c.type === "Article") {
              return {
                type: c.type,
                contentId: c.content?._id,
                categoryId: c.content?._id,
        ...
        const othersectionData = res.payload?.content?.otherSections.map(
          (c, i) => {
            return {
              type: "Image",
              contentId: c?._id,
        ...
        setHeroSection((prev) => [...herosectionData]);
        setOtherSections((prev) => [...othersectionData]);
        // setHeroSection((prev) => [...herosectionData, ...othersectionData]);
      }
    });
  ...
  return (
    <>
      ...
      <div className="w-full flex items-start lg:gap-3 flex-row h-auto">
        ...
        >
          <Header />
          {loading || articleLoading || homePageLoading ? (
                  ...
                  >
                    ...
                    {(articles.length > 0 || images.length > 0) &&
                      imageAndArticles.map((content, i) => (
                        <option
                          selected={heroSection.some(
                            (item) =>
                              item?.contentId === content?.contentId &&
                              item?.order == 1 &&
                              item?.from === "heroSection"
                          )}
                  ...
                  >
                    ...
                    {images.length > 0 &&
                      imageAndArticles
                        ...
                        .map((content, i) => (
                          <option
                            selected={otherSections.some(
                              (item) =>
                                item?.contentId === content?.contentId &&
                                item?.order == 1 &&
                                item?.from === "otherSection"
                            )}
                            value={content?.contentId}
                            key={i}
                          >
